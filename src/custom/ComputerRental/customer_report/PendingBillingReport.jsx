    ...
    <tr className="bg-white even:bg-gray-50 hover:bg-blue-50 transition text-sm">
      ...
      <td className="px-3 py-2 min-w-[170px]">
        <div className="font-medium text-gray-600">
          ...
          {row.billing_type && (<span className="text-xs text-gray-500">{`[ ${row.billing_type} ]`}</span>)}
          {row.billing_type && row.billing_type == 'contract' && (<span className="text-xs text-gray-500">{`[ ${row.contract_start || '-'} - ${row.contract_end || '-'} ]`}</span>)}
        </div>
