...
import { List, LucideMail, LucideSend, Pencil, Plus, Trash2 } from 'lucide-react';
import { getDaynamicData } from '@utils/utils';
import {defaultFormTemplate,directSchemaTemplate,fieldOverrides,columnLabels,defaultColumns,skipFields,filterSkipFields} from '../API/ChargersSchema';
...
const ChargersListView = () => {
  ...
  const { openChargerCreate } = useChargerCreateStore();
  ...
  setSelectedLeadId(rowData.record_id)
   openChargerCreate({payload:rowData},(result) => {
    refreshFilterList();
  });

};
const handleDelete =async (rowData) => {
  if (window.confirm(`Delete "${rowData.charger_serial_no}"?`)) {
          await fetchJson(`${API_BASE_URL}/chargerMaster/multiplechargerChangeStatus`, {
              method: 'POST',
              body: JSON.stringify({ action: "delete",menuId:"",list:rowData.id })
          });
          refreshFilterList();
      }
};
  ...
  return (
    ...
    <div className="p-4">
      ...
      <DynamicTable
        ...
        recordsApi={{
        url: `${API_BASE_URL}/chargerMasterList`,
        method: 'POST',
        body:filterData,
        }}
        ...
        bulkStatusAPI={`${API_BASE_URL}/chargerMaster/multiplechargerChangeStatus`}
        handleRefresh={refreshFilterList}
...
export default ChargersListView;