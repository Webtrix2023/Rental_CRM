...
function ReplacementPending() {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            const res = await fetchJson(`${API_BASE_URL}/replacement_pending_assets`, {
                method: 'POST',
                body: JSON.stringify({})
            }
            );
            ...
            if (res.flag === "S") {
                setAssets(data)
                console.log('replacement_pending_assets : ', assets);
            }else{
                setAssets([])
            }
        }
    ...
    return (
        <div className="rounded-xl shadow border border-gray-200 px-3 bg-white pt-7 w-full flex flex-col">
